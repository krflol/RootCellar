...
- CLI/batch dashboard: throughput, failure by command, repro mismatch rate.
  ...
  - Contract drift canaries: `python/validate_batch_schema_canaries.py` verifies representative schema-regression failures; migration process is defined in [[docs/RootCellar/04-Observability/Artifact Schema Migration Playbook]].
  - Dual-read migration drills: `python/validate_batch_dual_read_migration.py` verifies producer-major upgrades can be safely overlapped and rolled back before schema rollout across full artifact families, with CI subset control via `ALERT_POLICY_SCHEMA_MIGRATION_DRILL_ARTIFACTS`, staged-wave control via `ALERT_POLICY_SCHEMA_MIGRATION_DRILL_WAVE_SPEC`, fault-injection control via `ALERT_POLICY_SCHEMA_MIGRATION_DRILL_FAULT_INJECTION_ENABLED`/`ALERT_POLICY_SCHEMA_MIGRATION_DRILL_FAULT_SCENARIOS`, and diagnostics output artifact `ci-batch-schema-migration-drill.json`.
  - Migration policy dry-run checks: `python/validate_batch_migration_policy_dry_run.py` asserts CI policy parsing rejects invalid staged-wave specs and unsupported fault-scenario keys (`ALERT_POLICY_SCHEMA_MIGRATION_DRY_RUN_POLICY_VALIDATION_ENABLED`).
