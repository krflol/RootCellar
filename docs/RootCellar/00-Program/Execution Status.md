...
31. Baseline test suite passing in offline mode.
...
- `rg -n "schema-.*-fallback|ALERT_POLICY_SCHEMA_MIGRATION_DRILL_VALIDATION_ENABLED|ALERT_POLICY_SCHEMA_MIGRATION_DRILL_ARTIFACTS|ALERT_POLICY_SCHEMA_MIGRATION_DRILL_WAVE_SPEC|ALERT_POLICY_SCHEMA_MIGRATION_DRILL_FAULT_INJECTION_ENABLED|ALERT_POLICY_SCHEMA_MIGRATION_DRILL_FAULT_SCENARIOS|Run dual-read migration drills|ci-batch-schema-migration-drill.json|alert_policy_schema_migration_drill_validation_enabled|alert_policy_schema_migration_drill_artifacts|alert_policy_schema_migration_drill_wave_spec|alert_policy_schema_migration_drill_fault_injection_enabled|alert_policy_schema_migration_drill_fault_scenarios|schema_migration_drill_report_generated|validate_batch_dual_read_migration.py" .github/workflows/batch-recalc-nightly.yml python/validate_batch_adapter_contracts.py python/validate_batch_dual_read_migration.py`: pass (dual-read fallback + migration-drill artifact-subset/staged-wave/fault-injection/report integration markers present).
