...
# Optional (Windows + Microsoft Excel installed): regenerate curated verified-Excel samples.
...
python python/verify_excel_interop.py --workspace . --workdir ./target/excel-interop-gate --corpus-dir ./target/excel-interop-corpus --corpus-manifest ./target/excel-interop-corpus/manifest.json --max-corpus-files 32 --require-corpus-fixture minimal.xlsx --require-corpus-fixture formula.xlsx --require-corpus-fixture dangling-edge.xlsx --require-corpus-fixture unknown-part.xlsx --require-corpus-fixture styles.xlsx --require-corpus-fixture comments.xlsx --require-corpus-fixture chart.xlsx --require-corpus-fixture defined-names.xlsx --report ./target/excel-interop-gate-report.json
...
- Interop CI gate now enforces curated-sample coverage (`EXCEL_INTEROP_MIN_EXCEL_AUTHORED_SAMPLES=17`) while still requiring the deterministic generated fixture set.
- Interop CI gate enforces curated feature coverage (`formulas`, `styles`, `comments`, `charts`, `defined_names`, `tables`, `merged_cells`, `data_validation`, `conditional_formatting`, `external_links`, `pivot_tables`, `query_connections`, `sheet_protection`, `hyperlinks`, `workbook_protection`, `print_settings`, `calc_chain`) so curated corpus runs exercise broader compatibility paths.
- Interop corpus assembly now supports provenance-aware policy (`interim_openpyxl` vs `verified_excel`) and verified-Excel minimums (`EXCEL_INTEROP_MIN_VERIFIED_EXCEL_SAMPLES`, currently `17`).
...
## Next Build Slice
- Expand curated verified-Excel corpus beyond the current baseline set with additional real-world workbook variants (for example rich chart layouts and scenario-manager sheets) while keeping `EXCEL_INTEROP_MIN_VERIFIED_EXCEL_SAMPLES=17` green.
