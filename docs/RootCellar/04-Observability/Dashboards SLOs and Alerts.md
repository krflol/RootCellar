...
- CLI/batch dashboard: throughput, failure by command, repro mismatch rate.
  ...
  - Dual-read migration drills: `python/validate_batch_dual_read_migration.py` verifies producer-major upgrades can be safely overlapped and rolled back before schema rollout across full artifact families, with CI subset control via `ALERT_POLICY_SCHEMA_MIGRATION_DRILL_ARTIFACTS`.
