...
29. Baseline test suite passing in offline mode.
...
- `rg -n "schema-.*-fallback|ALERT_POLICY_SCHEMA_MIGRATION_DRILL_VALIDATION_ENABLED|ALERT_POLICY_SCHEMA_MIGRATION_DRILL_ARTIFACTS|Run dual-read migration drills|alert_policy_schema_migration_drill_validation_enabled|alert_policy_schema_migration_drill_artifacts|validate_batch_dual_read_migration.py" .github/workflows/batch-recalc-nightly.yml python/validate_batch_adapter_contracts.py python/validate_batch_dual_read_migration.py`: pass (dual-read fallback + migration-drill artifact-subset integration markers present).
