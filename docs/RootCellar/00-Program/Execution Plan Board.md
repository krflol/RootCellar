...
## Next Planned Items
1. Continue function parity expansion in higher-impact text/lookup families (`LEFT`, `RIGHT`, `MID`, `LOWER`, `UPPER`, `TRIM`, `SUBSTITUTE`, `REPLACE`, `CONCAT`, `TEXTJOIN`) with compatibility-focused recalc tests.
2. Continue parser/intern scheduler hot-path optimization with benchmark-backed validation on larger dependency graphs.
